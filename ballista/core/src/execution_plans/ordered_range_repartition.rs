...
mod tests {
    ...
    async fn end_to_end_conserves_rows_across_two_overlapping_partitions() {
        ...
        let part_a: Vec<(Option<f64>, i64)> =
            (0..20).map(|i| (Some((i * 2) as f64), i as i64)).collect();
        let part_b: Vec<(Option<f64>, i64)> = (0..20)
            .map(|i| (Some((i * 2 + 1) as f64), 100 + i as i64))
            .collect();
    ...
    async fn end_to_end_each_output_is_sorted() {
        let schema = schema_v2_id();
        let part_a: Vec<(Option<f64>, i64)> =
            (0..25).map(|i| (Some((i * 2) as f64), i as i64)).collect();
        let part_b: Vec<(Option<f64>, i64)> = (0..25)
            .map(|i| (Some((i * 2 + 1) as f64), 100 + i as i64))
            .collect();
        let exec = build_pipeline(&schema, vec![part_a, part_b], 4);
        let per_output = drain_concurrent(exec, session()).await;
    ...
    async fn end_to_end_outputs_are_range_disjoint() {
        let schema = schema_v2_id();
        let part_a: Vec<(Option<f64>, i64)> =
            (0..30).map(|i| (Some((i * 2) as f64), i as i64)).collect();
        let part_b: Vec<(Option<f64>, i64)> = (0..30)
            .map(|i| (Some((i * 2 + 1) as f64), 100 + i as i64))
            .collect();
        let part_c: Vec<(Option<f64>, i64)> = (0..30)
            .map(|i| (Some(((i * 2) + 100) as f64), 200 + i as i64))
            .collect();
    ...
    async fn execute_same_partition_twice_errors() {
        let schema = schema_v2_id();
        let exec = build_pipeline(&schema, vec![vec![(Some(1.0), 0)]], 3);
        let ctx = session();
