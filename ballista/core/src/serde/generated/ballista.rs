...
pub struct RuntimeStatsReport {
    ...
    /// position in the plan — pre-DRR gets one entry per input partition,
    /// post-DRR gets one per output sub-partition. Scheduler groups by
    /// `order_by` tag and aggregates.
    #[prost(message, repeated, tag = "2")]
...
pub struct RuntimeStatsPartitionEntry {
    ...
    ///
    /// TODO: `optional MinMaxState min_max` — for a lighter post-DRR mode
    /// where the bin-packer just needs (min, max, count) per sub-partition
    /// and a full T-Digest is overkill.
    #[prost(message, optional, tag = "3")]
